...
ggsave(
  ...
  dpi = 300
                     ...
                     # ------------------------------------------------------------------
# TTLL11 survival analysis and joint Cox model
# ------------------------------------------------------------------

ttll11_df <- expr_df %>%
  dplyr::select(sample, Cancer_Type, TTLL11) %>%
  group_by(sample, Cancer_Type) %>%
  summarise(
    TTLL11_vst = median(TTLL11, na.rm = TRUE),
    .groups = "drop"
  )

dat_ttll11 <- merged_df %>%
  left_join(ttll11_df, by = c("sample", "Cancer_Type")) %>%
  mutate(
  ...
  filter(
    ...
    time > 0,
    status %in% c(0, 1),
    is.finite(TTLL11_vst),
    is.finite(rf_tumor_score)
  ) %>%
  distinct(sample, Cancer_Type, .keep_all = TRUE)

message("N for TTLL11 survival: ", nrow(dat_ttll11),
        " | Events: ", sum(dat_ttll11$status == 1))

write.csv(
  dat_ttll11,
  file.path(out_dir, "ttll11_survival_analysis_dataset.csv"),
  row.names = FALSE
)

q1_ttll11 <- quantile(dat_ttll11$TTLL11_vst, 0.25, na.rm = TRUE)
q3_ttll11 <- quantile(dat_ttll11$TTLL11_vst, 0.75, na.rm = TRUE)

ttll11_q <- dat_ttll11 %>%
  mutate(
    grp = case_when(
      TTLL11_vst <= q1_ttll11 ~ "Low TTLL11 (Q1)",
      TTLL11_vst >= q3_ttll11 ~ "High TTLL11 (Q4)",
      TRUE ~ NA_character_
    )
  ) %>%
  filter(!is.na(grp)) %>%
  mutate(
    grp = factor(
      grp,
      levels = c("High TTLL11 (Q4)", "Low TTLL11 (Q1)")
    )
  )

message(sprintf(
  "TTLL11 quartiles: Q1 = %.4f | Q3 = %.4f",
  q1_ttll11,
  q3_ttll11
))

ttll11_q_summary <- ttll11_q %>%
  group_by(grp) %>%
  summarise(
    mean_TTLL11 = mean(TTLL11_vst, na.rm = TRUE),
    events = sum(status),
    n = n(),
...
print(ttll11_q_summary)

write.csv(
  ttll11_q,
  file.path(out_dir, "ttll11_top_bottom_quartile_survival_data.csv"),
  row.names = FALSE
)

write.csv(
  ttll11_q_summary,
  file.path(out_dir, "ttll11_top_bottom_quartile_summary.csv"),
  row.names = FALSE
)

fit_ttll11_q <- survfit(Surv(time_years, status) ~ grp, data = ttll11_q)
cox_ttll11_q <- coxph(Surv(time, status) ~ grp, data = ttll11_q)

print(summary(cox_ttll11_q))

cox_sum <- summary(cox_ttll11_q)

hr <- cox_sum$coef[1, "exp(coef)"]
hr_l <- cox_sum$conf.int[1, "lower .95"]
hr_u <- cox_sum$conf.int[1, "upper .95"]
p_hr <- cox_sum$coef[1, "Pr(>|z|)"]
...
hr_label <- sprintf(
  ...
  hr_l,
  hr_u,
  ifelse(p_hr < 1e-3, "<0.001", signif(p_hr, 2))
)

pal_ttll11 <- c("#B45757", "#4F7052")

km_ttll11 <- ggsurvplot(
  fit_ttll11_q,
  data = ttll11_q,
  pval = TRUE,
  conf.int = TRUE,
  ...
  censor = FALSE,
  surv.size = 1.4,
  legend.labs = levels(ttll11_q$grp),
  legend.title = "TTLL11 expression",
  title = "Pan-cancer survival: top vs bottom TTLL11 quartiles",
  xlab = "Time (years)",
  ylab = "Survival Probability",
  xlim = c(0, 10),
  break.x.by = 1,
...
s5_ttll11 <- summary(fit_ttll11_q, times = 5)

fiveyr_ttll11 <- data.frame(
  strata = s5_ttll11$strata,
  surv = s5_ttll11$surv
) %>%
  filter(!is.na(surv)) %>%
  mutate(label = sprintf("%.1f%%", 100 * surv))

km_ttll11$plot <- km_ttll11$plot +
  scale_x_continuous(
    expand = c(0, 0),
    limits = c(0, 10),
    breaks = seq(0, 10, 1)
...
km_ttll11$table <- km_ttll11$table +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    axis.line = element_line(color = "black", linewidth = 0.8),
...
# ------------------------------------------------------------------
# Joint Cox model: TTLL11 expression + RF tumor score
# ------------------------------------------------------------------

fit_joint <- coxph(
  Surv(time, status) ~ scale(TTLL11_vst) + scale(rf_tumor_score),
  data = dat_ttll11
)
...
joint_cox_table <- broom::tidy(
  ...
  mutate(
    term = recode(
      term,
      "scale(TTLL11_vst)" = "TTLL11",
      "scale(rf_tumor_score)" = "RF score"
    )
  )

write.csv(
  joint_cox_table,
  file.path(out_dir, "joint_cox_TTLL11_RFscore.csv"),
  row.names = FALSE
)

df_forest <- joint_cox_table %>%
  mutate(
    label = sprintf(
      "HR %.2f (%.2f-%.2f)\np = %s",
      estimate,
      conf.low,
      conf.high,
      ifelse(p.value < 1e-3, "<0.001", signif(p.value, 2))
    )
  )
...
p_forest <- ggplot(
  ...
  geom_pointrange(linewidth = 1.1) +
  geom_hline(yintercept = 1, linetype = "dashed") +
  geom_text(aes(label = label), hjust = -0.1, size = 4) +
  coord_flip() +
  labs(
    title = "Joint Cox model",
    x = NULL,
  ...
  theme(
    ...
    axis.text = element_text(color = "black"),
    axis.title = element_text(color = "black", face = "bold"),
    plot.margin = ggplot2::margin(15, 60, 15, 15)
  )

print(p_forest)

ggsave(
  file.path(out_dir, "joint_cox_TTLL11_RFscore_forest.png"),
  p_forest,
  width = 7,
  height = 4,
  dpi = 300
